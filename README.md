...
## 2026-09-04：I2C 角度反馈失效保护

- AS5600 单次 I2C 事务超时从 100 ms 降为 2 ms；角度任务发生读失败时立即清除 `foc_latest_angle_valid`，并等待 2 ms 后重试，避免持续占用 CPU。
- 电流环把角度有效性作为硬门限：角度无效或 `angle_age_us > 1500` 时，PWM 输出回到 50/50/50，d/q PI 状态清零，`iq_ref` 清零；角度恢复后再从零电流参考重新爬升。
- AS5600 读失败会清除速度估计初始化状态，恢复后的第一次有效采样只建立基准，不跨越 I2C 故障区间计算速度。
- 角度年龄使用有符号差值，避免角度任务与电流任务并发更新时发生无符号下溢。
- 这是标准闭环故障处理：传感器反馈失效时进入安全状态，而不是继续用旧角度进行 Park/解耦。
